                ...
                <div className="grid lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-4 space-y-8">
                        <div className="bg-surface p-6 sm:p-8 rounded-[32px] border border-border-light shadow-xl shadow-black/5">
                            <h3 className="text-xl font-bold text-text-primary mb-8 px-2 sm:px-0 text-center sm:text-left">Conquistas</h3>
                            <div className="grid grid-cols-2 sm:grid-cols-1 gap-6 sm:gap-8">
                                {[
                                    ...
                                    <div key={stat.label} className="group text-center sm:text-left px-2">
    ...
    return (
        ...
        >
            <button
                ...
                className="w-full flex items-center gap-6 p-8 text-left outline-none"
            >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 transition-colors duration-500 ${expanded ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-surface-secondary text-text-tertiary'}`}>
                    {exp.type === 'teaching' ? <HiAcademicCap size={28} /> :
                        exp.type === 'hospital' ? <HiOfficeBuilding size={28} /> :
                            <HiBriefcase size={28} />}
                </div>

                <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-1">
                        <h3 className={`text-lg font-bold transition-colors ${expanded ? 'text-primary' : 'text-text-primary'}`}>
                            {exp.institution}
                        </h3>
                        {expanded && (
                            <span className="text-[10px] font-bold text-primary px-3 py-1 rounded-full bg-primary/10">ATUAL</span>
                        )}
                    </div>
                    <p className="text-xs font-bold text-text-tertiary uppercase tracking-widest">{exp.period}</p>
                </div>

                <div className={`p-2 rounded-full transition-all duration-500 ${expanded ? 'bg-primary/10 text-primary rotate-180' : 'bg-surface-secondary text-text-tertiary'}`}>
                ...
                {expanded && (
                    ...
                    >
                        <div className="px-8 pb-8 pt-0">
                            <div className="h-[1px] w-full bg-border-light mb-6" />
                            <ul className="space-y-4">
                                {exp.roles.map((role) => (
                                    <li key={role} className="flex items-start gap-4">
                                        ...
                                        <span className="text-base text-text-secondary font-medium leading-relaxed">{role}</span>
                                    </li>
                                ))}
                            </ul>
                        </div>
